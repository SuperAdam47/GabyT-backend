...
module.exports =  {
  custom: q => {
    ...
    if (q.link) pre += `With help of the 'reference' page${q.doc ? `, as applied to the ${sub(q)}` : ""}: q => `
    else if (q.doc) pre += `In reference to the ${sub(q)}: q => `
    return pre + q.value
  },
  ...
  poem: q => `Write me a poem on this subject: ${q.value}`,
